...
double noise(const base_node p) {
...
void matlab_export_truth(const std::vector<double> u) {

  std::ofstream blabla0;
  blabla0.open("./MATLAB/Coeff_truth.txt", ios::out|ios::trunc);
  blabla0.close();

  std::ofstream blabla;
  blabla.open("./MATLAB/Coeff_truth.txt", ios::out|ios::app);
...
void compute_supergradient(std::vector<double> &Supergrad, const std::vector<double> weights, const std::vector<double> weights_trans,
			   ...
			   std::vector<double> &mu, std::vector<double> &u, const std::vector<double> udata, const std::vector<double> utruth) {

	size_type Ndata = utruth.size();
	...
	Supergrad[weights.size()-1] = 0.0; // For avoiding trivial NN to be critical points
...
void Super_Nesterov_algorithm(std::vector<double> &weights, std::vector<double> &weights_trans, std::vector<double> &Supergrad, 
	...
	while ((n<=Nmax) && (std::min(tol,100.0*difftol)>super_tol)) {
		...
		// Gradient
		compute_supergradient(Supergrad, sw, sw_trans, mesh, mim, mf, y, ydata, Nmax_subnesterov, step_size_nesterov, tol_subnesterov, mu, u, udata, utruth);
...
int main(int argc, char *argv[]) {
  ...
  getfem::interpolation_function(mf, vec_noise, noise);
  for (size_type k=0; k<Ndata; ++k) {
  ...
  for (size_type l=0; l<L; ++l) {
	weights[l] = -1.0*(double(l&1)-0.5)*(-2.0);
  }
  std::vector<double> weights_trans(L);
  for (size_type l=0; l<L; ++l) {
	weights_trans[l] = 0.0;//(double(l&1)-0.5)*(1.0);
  }
  ...
  coefflearnedL202.open("./MATLAB/CoefflearnedNoise_validation.txt", ios::out|ios::trunc);
  coefflearnedL202.close();
  std::vector<double> udata2(Ndata2);

  std::vector<double> vec_noise2(nb_dof);
  getfem::interpolation_function(mf, vec_noise2, noise);
  for (size_type k=0; k<Ndata2; ++k) {
	gmm::add(vec_noise, ydata2[k]);	
  }
  ...
  for (size_type k=0; k<Ndata2; ++k) {
	...
	subproblem_Nesterov_L2(udata2[k], ydata2[k], mesh, mim, mf, Nmax_subnesterov, step_size_nesterov, tol_subnesterov);
  	...
  	//matlab_export_coeff2(udata2[k]);
  		...
  		evolcoeffNN02.open("./MATLAB/CoefflearnedNoise_validation.txt", ios::out|ios::app);
